...
def render_trail_list():
    """List all trails across all parks."""
    ...
    for park in parks:
        ...
        if resp.status_code == 200:
            ...
            st.warning(
                f"Backend returned {resp.status_code} for trails in park {park['name']}."
            )
...
with trail_review_tab:
    ...
    def render_trail_reviews() -> None:
        """Render user's trail reviews and allow submitting new ones."""
        ...
        # Build list of all trails for dropdown
        trails: list[tuple[str, str]] = []
        parks = get_parks()
        for park in parks:
            park_id = park.get("park_id")
            if not park_id:
